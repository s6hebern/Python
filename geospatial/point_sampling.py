...
Description:
-----------
Sample a raster dataset at specific coordinates, indicated by a spatial point vector file. The tool 
will pay attention to spatial reference and is able to not only sample the exact pixel for each 
point, but also a window with given radius around it, where different statistical measures can be 
calculated. Raster attributes will be added as new attributes.
...
""", conflict_handler='resolve')
        ...
        group.add_option('-p', '--points', dest='points', type='str',
                         help='Sampling points (vector format)')
        ...
        group.add_option('-d', '--dismiss', dest='dismiss',
                         help='Raster value to dismiss. It will be ignored for statistical '
                              'calculation.')
        group.add_option('-n', '--names', dest='names', type='str',
                         help='Field names for the target attributes. Must not exceed 10 characters'
                              ' and only contain '
                              '"a-z,A-Z,0-9,_". If a field already exists, its values will be '
                              'overwritten!')
        ...
        group.add_option('-o', '--output', dest='output', type='str',
                         help='Output file, if desired. If not given, the new attributes will be'
                              ...
                              'dataset. The file extension needs to match the one from the input'
                              ' dataset, since they '
    ...
    def check_options(self):
        ...
        if not options.points:
            parser.print_help()
            raise IOError('No sampling points!')
    ...
    def run(self):
        ...
        self._points, self._image = self._match_projection(self._points, self._image,
                                                           match=self._crs)
    ...
    def _match_projection(self, shape, raster, match='raster'):
        # get raster information
        ds = gdal.Open(raster)
        ...
        if epsg_raster != epsg_shape:
            if match == 'raster':
                ...
                warnings.warn('Need to re-project points! New output dataset will be {ds}'.format(
                    ds=proj_shape))
                cmd = 'ogr2ogr -overwrite -f "ESRI Shapefile" -t_srs EPSG:{e} {ds} {s}'.format(
                    e=epsg_raster, ds=proj_shape, s=shape)
                self.run_cmd(cmd)
                shape = proj_shape
                self._temp = None
            else:
